...
if isvector(rp) && all(ismember(rp, [1, 2, 3, 4]))
    ...
    for i = 1:length(rp)
        ...
        elseif rp(i) == 4
            ...
            plot(1:N, nr, colors{rp(i)}, 'DisplayName', methods{rp(i)});
            CATCH ME 
               disp('We encountered an issue due to ichol. Please restart and choose a valid matrix. If the problem persists, avoid using the pr_cg method.')
               return
            end
        end
    ...
    title('Comparison of Gradient Descent Methods Case Full Sparse Matrixe');
    grid on;
    ...
    title('Execution Time of Gradient Descent Methods Case Full Sparse Matrix');
    ...
    title('Memory Usage of Gradient Descent Methods Case Full Sparse Matrixe');
    figure;
    hold on;
    colors = {'k', 'r', 'g', 'b'}; % Couleurs pour les méthodes
    methods = {'CG', 'BiCG', 'BiCG_STAB', 'CG_PREC'}; % Noms des méthodes
    ...
    for i = 1:length(rp)
        tic; % Début du chronométrage
        if rp(i) == 1
            [x, r, p, nr, N, whos_] = cg(sparse(A), b, x0, taux, itmax);
            plot(1:N, nr, colors{rp(i)}, 'DisplayName', methods{rp(i)});
        elseif rp(i) == 2
            [x, r, p, nr, N, whos_] = bicg(sparse(A), b, x0, taux, itmax);
            plot(1:N, nr, colors{rp(i)}, 'DisplayName', methods{rp(i)});
        elseif rp(i) == 3
            [x, r, p, nr, N, whos_] = bcgstab(sparse(A), b, x0, taux, itmax);
            plot(1:N, nr, colors{rp(i)}, 'DisplayName', methods{rp(i)});
        elseif rp(i) == 4
            try
            [x, r, p, nr, N, whos_] = pr_cg(sparse(A), b, x0, taux, itmax);
            plot(1:N, nr, colors{rp(i)}, 'DisplayName', methods{rp(i)});
             CATCH ME 
               disp('We encountered an issue due to ichol. Please restart and choose a valid matrix. If the problem persists, avoid using the pr_cg method.')
               return
        ...
        % Calcul de la mémoire utilisée pour la méthode
        memory_usage_comp(i) = sum([whos_.bytes]) / 1e6; % Convertir en Mo
    ...
    xlabel('Iteration');
    ylabel('Residual Norm');
    title('Comparison of Gradient Descent Methods Case Compressed Sparse Matrix');
    grid on;
    ...
    xlabel('Methods');
    ylabel('Execution Time (s)');
    title('Execution Time of Gradient Descent Methods Case Compressed Sparse Matrix');

    % Ajout d'un graphe pour la consommation mémoire
    figure;
    bar(1:length(rp), memory_usage_comp);
    set(gca, 'XTickLabel', methods(rp));
    xlabel('Methods');
    ylabel('Memory Usage (MB)');
    title('Memory Usage of Gradient Descent Methods Case Compressed Sparse Matrix');
